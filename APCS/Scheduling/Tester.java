...
class Master {
	...
	public void check(Course x, int sect) {
		...
		for (int i = 0; i < x.roster.size(); i++)// for each student on the roster
		{
			...
			{
				// populates temp1 w periods, for each kid, doesn't populate with course we are
				// checking (thats the part after &&)
				// this is where we have to make the adjustment for double scheduled sections
				// such as (3,4) and (3,4) so that it doesn't add twice
				if (x.roster.get(i).courses.get(j).posted && !x.equals(x.roster.get(i).courses.get(j))) {
					...
					for (int q = x.roster.get(i).temp1.get(x.roster.get(i).temp1.size() - 1).size() - 1; q > 0; q--)// working
																													// on
																													// this,
																													// very
																													// unsure
					{
						for (int r = q - 1; r >= 0; r--) {
							if (x.roster.get(i).temp1.get(x.roster.get(i).temp1.size() - 1).get(q)
									.equals(x.roster.get(i).temp1.get(x.roster.get(i).temp1.size() - 1).get(r))) {
	...
	void check2(Course w, Course y){
		if (w.sections() ==1 && w.lab==1){   //if x has one regular section
     		for (int i = 1; i<=15;  i=i+2){
				System.out.println((i+1)/2);
				ArrayList<Integer> try1 = new ArrayList<Integer>();
				try1.add(i);
				try1.add(i+1);
				w.trying.add(try1);   //adding (1,2), then (3,4) etc to the trying list
				// next call the checking method with y
				x.check(y);
			}
		}
	}
